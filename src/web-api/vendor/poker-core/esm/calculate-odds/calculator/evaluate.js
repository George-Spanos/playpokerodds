import { CardGroup, HandRanks, Ranks } from './types.js';
import { uniqWith } from './util.js';
export function evaluate(cardGroup) {
    cardGroup.sortDesc();
    // group by rank
    ...
    if (flushSuit) {
        if (straightCardsCount === 5) {
            ...
            if (straightFlushCards.length >= 5) {
                ...
                if (isStraightFlush)
                    return { handRank: HandRanks.straightFlush, hand: new CardGroup(straightFlushCards.slice(index, index + 5)) };
            }
        }
        else if (straightCardsCount === 4 && straightMaxCardRank === Ranks.five) {
            const aceCards = cardGroup.cards.filter(card => card.suit === flushSuit && card.rank === Ranks.ace);
            if (aceCards.length > 0) {
                const straightFlushCards = cardGroup.cards.filter(card => card.suit === flushSuit && card.rank <= straightMaxCardRank);
                if (straightFlushCards.length === 4)
                    return { handRank: HandRanks.straightFlush, hand: new CardGroup(straightFlushCards.concat(aceCards[0]).slice(0, 5)) };
            }
        }
    }
    // quads
    if (quadRanks.length >= 1) {
        ...
        return { handRank: HandRanks.quads, hand: new CardGroup(quadCards.concat(otherCards).slice(0, 5)) };
    }
    // full house
    if (tripRanks.length === 1 && pairRanks.length >= 1) {
        ...
        return { handRank: HandRanks.fullHouse, hand: new CardGroup(tripCards.concat(pairCards)) };
    }
    else if (tripRanks.length > 1) {
        const tripCards = cardGroup.cards.filter(card => card.rank === tripRanks[0]);
        const pairCards = cardGroup.cards.filter(card => card.rank === tripRanks[1]);
        return { handRank: HandRanks.fullHouse, hand: new CardGroup(tripCards.concat(pairCards.slice(0, 2))) };
    }
    // flush
    if (flushSuit) {
        const flushCards = cardGroup.cards.filter(card => card.suit === flushSuit);
        return { handRank: HandRanks.flush, hand: new CardGroup(flushCards.slice(0, 5)) };
    }
    // straight
    if (straightCardsCount === 5) {
        const straightCards = uniqWith(cardGroup.cards.filter(card => card.rank <= straightMaxCardRank), (c1, c2) => c1.rank === c2.rank);
        return { handRank: HandRanks.straight, hand: new CardGroup(straightCards.slice(0, 5)) };
    }
    else if (straightCardsCount === 4 && straightMaxCardRank === Ranks.five) {
        const aceCards = cardGroup.cards.filter(card => card.rank === Ranks.ace);
        if (aceCards.length > 0) {
            const straightCards = uniqWith(cardGroup.cards.filter(card => card.rank <= straightMaxCardRank), (c1, c2) => c1.rank === c2.rank);
            return { handRank: HandRanks.straight, hand: new CardGroup(straightCards.concat(aceCards[0]).slice(0, 5)) };
        }
    }
    // trips
    if (tripRanks.length === 1) {
        ...
        return { handRank: HandRanks.trips, hand: new CardGroup(tripCards.concat(cards).slice(0, 5)) };
    }
    // two pair
    if (pairRanks.length >= 2) {
        ...
        return { handRank: HandRanks.twoPair, hand: new CardGroup(pairedHigherCards.concat(pairedLowerCards)
    ...
    if (pairRanks.length === 1) {
        ...
        return { handRank: HandRanks.pair, hand: new CardGroup(pairedCards.concat(unpairedCards).slice(0, 5)) };
    }
    // high card
    return { handRank: HandRanks.highCard, hand: new CardGroup(cardGroup.cards.slice(0, 5)) };
}
