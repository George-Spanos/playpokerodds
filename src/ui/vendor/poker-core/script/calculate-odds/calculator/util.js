...
exports.calculateMaxIterations = calculateMaxIterations;
...
function validateInput(input) {
    ...
    if ('board' in input) {
        ...
        const cardGroup = new types_js_1.CardGroup(input.board);
        if ('boardSize' in input) {
            ...
            if (cardGroup.cards.length > 5)
                throw new Error(`"board" cannot contain more than 5 cards. Invalid: ${input}`);
        }
    }
    // iterations
    ...
    if ('hands' in input) {
        ...
        for (const hand of input.hands) {
            const cardGroup = new types_js_1.CardGroup(hand);
            if (!('handSize' in input)) {
...
function calculateMaxIterations(input) {
